...
class ConcreteDropout(Wrapper):
	...
	def build(self, input_shape=None):
		...
		# This is insanely hacky, but it's the best way to deal with Keras
		def dropout_regularizer(p_logit):
			p = K.sigmoid(p_logit[0])
			dropout_regularizer = p * K.log(p)
			dropout_regularizer += (1. - p) * K.log(1. - p)
			dropout_regularizer *= self.dropout_regularizer
			dropout_regularizer += self.weight_regularizer * K.sum(K.square(self.layer.kernel)) / (1. - p)
			return dropout_regularizer
